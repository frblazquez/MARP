...
template<class T>
bool splay_tree<T>::insert(T const& elem) {  return insert(root, nullptr, elem); }
...
template<class T>
bool splay_tree<T>::insert(Node* &node, Node* father, T const& elem)
{
  if(node == nullptr)
  {
  ...
  else if(node->elem == elem) return false;
  else if(node->elem >  elem) return insert(node->left, node, elem);
  else                        return insert(node->right, node, elem);
}
...
bool splay_tree<T>::erase(Node* &node, T const& elem)
{
  ...
  else if(node->elem > elem)  return erase(node->left,  elem);
  else if(node->elem < elem)  return erase(node->right, elem);
  else
  {
...
{
  if(node == nullptr)         return false;
  else if(node->elem == elem) return true;
  else if(node->elem >  elem) return search(node->right, elem);
  else                        return search(node->left,  elem);
}
